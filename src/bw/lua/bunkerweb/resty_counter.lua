-- Per-worker batched counter. Increments are accumulated in a worker-local
-- table, then flushed to a shared dict via ngx.timer.every. Keeps per-request
-- write amplification out of the SHM zone spinlock.
--
-- Adapted from nginx-lua-prometheus (knyar/nginx-lua-prometheus, MIT) via the
-- BunkerWeb Pro monitoring plugin. Generic - does not depend on any plugin.
...
function _M.new(shdict_name, sync_interval, error_metric_name)
	...
	if sync_interval then
		...
		if not timer_started[shdict_name] then
			...
			local ok, err = ngx.timer.every(sync_interval, sync, self)
			if ok then
				timer_started[shdict_name] = true
			else
				ngx.log(ngx.ERR, "failed to start counter sync timer for ", shdict_name, ": ", err)
			end
		end
	end
