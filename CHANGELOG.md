...
- [BUGFIX] `modsecurity`/`ui`/`antibot`: fix `USE_MODSECURITY_GLOBAL_CRS=yes` blocking UI and antibot POSTs with 403. Move chained exclusions to phase 1, lowercase + `:port`-tolerant `Host` regex with `re.escape`, and `modsecurity off;` on default-server UI proxy locations. (Fixes #3118)
- [BUGFIX] `database`: back-fill missing `bw_settings` defaults from `settings.json` at read time so directives like `client_body_timeout` no longer render empty after a desynced upgrade. Logs one WARNING per affected setting. (Fixes #3450)
- [BUGFIX] `errors`: revert rc4 `return 444;` on `@bwerror*` handlers - the deny path already bypasses `error_page`, so the gate only broke legitimate error rendering. Operators wanting stealth can use `INTERCEPTED_ERROR_CODES=""` or `ERRORS=`. (Fixes #3490, reverts #3448)
- [UI] Reports/Bans: CSV/Excel exports now include every column and honor active search + SearchPanes filters. (Fixes #3489)
- [UI] Service edit page: restore non-UI-method settings and template defaults on advanced/raw save; keep the `IS_DRAFT=` raw line and the draft toggle in sync.
- [BUGFIX] `metrics`/`badbehavior`/`limit`/`authbasic`: fix per-worker RAM leak and `no memory` log floods on high-cardinality multi-site setups. Bound the metrics LRU (new `METRICS_LRU_SIZE`, default 10000), replace unbounded per-IP/URL/user counters with bounded Top-N admission via the new `bunkerweb.top_n` lib, migrate UI consumers, SCAN+UNLINK legacy `metrics:*_table_*` keys, and use `LTRIM` instead of LPOP-loop. Lowers `METRICS_MAX_BLOCKED_REQUESTS_REDIS` default 100000 -> 10000.
